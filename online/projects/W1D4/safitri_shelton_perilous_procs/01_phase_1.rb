...
def exactly?(arr, num, &prc)
    results = arr.select { |n| prc.call(n) }
...
# should return a new array where elements of the original array are removed if they
# return true when given to the block. Solve this using Array#each.

def filter_out(arr, &prc)
    results = []
    arr.each { |n| results << n if !prc.call(n) }
    results
end
...
# to the block. If no element of returns true, then the method should return nil. 
# Solve this using Array#each.

def first_index(arr, &prc)
    results = []
    arr.each_with_index { |n, i| results << i if prc.call(n) }
    results[0]
end
