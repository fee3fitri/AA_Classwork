...
def xnor_select(arr, blk1, blk2)
    arr.select { |n| (blk1.call(n) && blk2.call(n)) or (!blk1.call(n) && !blk2.call(n)) }
end
...
# a new array. That is, write a method filter_out! that accepts an array and a block as
# args. The method should remove elements of the input array that return true when given
# to the block. Solve this without using Array.reject!.

def filter_out!(arr, &prc)
    arr.each { |n| arr.delete(n) if prc.call(n) }
    arr
end
...
# ----- multi_map -----
# Write a method multi_map that accepts an array, an optional number (n), and a block
# as arguments. The method should return a new array where each element of the original
# array is repeatedly run through the block n times. If the number argument is not
# passed in, then the the elements should be run through the block once.
